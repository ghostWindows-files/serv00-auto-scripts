  ...
  for (const account of accounts) {
    ...
    try {
      ...
      if (isLoggedIn) {
        ...
        console.log(`账号 UnknowAccount 于北京时间 ${nowBeijing}（UTC时间 ${nowUtc}）登录成功！`);
      } else {
        console.error(`账号 UnknowAccount 登录失败，请检查账号和密码是否正确。`);
      }
    } catch (error) {
      console.error(`账号 UnknowAccount 登录时出现错误: ${error}`);
    } finally {
      // 关闭页面和浏览器
      await page.close();
      await browser.close();
